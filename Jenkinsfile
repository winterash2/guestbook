...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="winterash2/cicd_guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'master', url:'https://github.com/winterash2/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
    ...
    post { 
        ...
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'winterash2.j@gmail.com')
